...
$consulta1 = $con->query("SELECT alumno.id, alumno.nombreAlum, alumno.apellidoAlum, alumno.legajoAlumno FROM alumno, alumnocursoactual, curso WHERE curso.id = '$id_curso' AND alumnocursoactual.curso_id = curso.id AND alumnocursoactual.fechaDesdeAlumCurAc = curso.fechaDesdeCursado AND alumnocursoactual.fechaHastaAlumCurAc = curso.fechaHastaCursado AND alumnocursoactual.alumno_id = alumno.id");
$alumnos = array();
